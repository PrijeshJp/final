<!DOCTYPE html>
<html lang="en" class="no-js">
<head>  
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Oracle Business Intelligence Enterprise Edition OBIEE customization and development outsourcing - Minuscule Technologies</title>
    <link rel="shortcut icon" href="favicon.ico" />
    <meta name="description" content="OBIEE customization and development outsourcing solutions for reporting, querying, analytic, OLAP, dashboard and scorecard functionalities on OBIEE.">
    <meta name="author" content="">
    <link rel="canonical" href="http://www.minusculetechnologies.com/obiee.html" />
    <meta property="og:locale" content="en_US" />
    <meta property="og:type" content="website" />
    <meta property="og:title" content="Oracle Business Intelligence Enterprise Edition OBIEE customization and development outsourcing - Minuscule Technologies" />
    <meta property="og:description" content="OBIEE customization and development outsourcing solutions for reporting, querying, analytic, OLAP, dashboard and scorecard functionalities on OBIEE" />
    <meta property="og:url" content="http://www.minusculetechnologies.com/obiee.html" />
    <meta charset="utf-8">
    <?php
        include "script.php";
	?>
</head>
<!-- Body -->
<body>
    <?php
        include ("nav.php");
    ?>
    <!--========== PROMO BLOCK ==========-->
    <img src="img/1920x1080/oracle.jpg" class="img-responsive res">
    <!--========== END PROMO BLOCK ==========-->
    <!--========== PAGE CONTENT ==========-->
    <div class="container blogpad">
        <div class="wow animated " style="visibility: visible;">
            <h2><span class="item active">Oracle Business Intelligence (OBIEE)</span></h2>
        </div>
        <!-- about us slider -->
        <div class="skill-wrap clearfix">
            <div class="wow animated" style="visibility: visible;">
                <p class="lead fontSizeChange">
                    We have developed a series of innovative implementation, upgrade and consulting services designed to maximize the return on existing and future Oracle BI investments. We can help you unlock the powerful and comprehensive features that come with OBIEE through our custom solutions and consulting.
                    <br>
                </p>
            </div>
        </div>
        <div class="skill-wrap clearfix">
            <div class="wow animated fontSizeChange" style="visibility: visible;">
                OBIEE customization
                <p class="lead fontSizeChange">
                    Our team can support you to customize the OBIEE that suits your organisation needs along with your own branding customization for both web and mobile solutions.
                    <br>
                </p>
            </div>
        </div>
        <div class="skill-wrap clearfix">
            <div class="wow animated fontSizeChange" style="visibility: visible;">
                OBIEE Implementation
                <p class="lead fontSizeChange">
                    We provide a full end-to-end OBIEE Implementation Service based upon our proven implementation methodologies. Our approach of agile and iterative method ensures engagement at all levels of your organization, from the earliest planning stages through going live, driving successful user adoption throughout the enterprise thus by maximizing the ROI.
                    <br><br>
                    Each of the OBIEE implementation is accompanied by our expert review, real-time task management and documentation service, as well as access to the industry's leading Oracle BI thought leaders. We are here to assist you at any phase of your Oracle BI deployment and can provide your organization with the resources needed to make the deployment faster and quicker at optimal cost.
                </p>
            </div>
        </div>
        <div class="skill-wrap clearfix">
            <div class="wow animated" style="visibility: visible;">
                OBIEE Mobile app customization
                <p class="lead fontSizeChange">
                    This service from Minuscule help in customizing your OBIEE mobile apps based on your business needs and also we will customize the OBIEE app with your own branding and logo to give your organization the opportunity to take advantage of the newest capabilities offered in OBIEE.
                    <br>
                </p>
            </div>
        </div>
        <ul class="keyFeatureList wow animated" data-wow-delay="600ms" data-wow-duration="1000ms" style="visibility: visible; animation-duration: 1000ms; animation-delay: 600ms;">
            <li><i class="fa fa-hand-o-right fontSizeChange" aria-hidden="true"></i> OBIEE Development</li>
            <li><i class="fa fa-hand-o-right fontSizeChange" aria-hidden="true"></i> OBIEE Customization</li>
            <li><i class="fa fa-hand-o-right fontSizeChange" aria-hidden="true"></i> OBIEE Integration</li>
            <li><i class="fa fa-hand-o-right fontSizeChange" aria-hidden="true"></i> OBIEE Migration</li>
            <li><i class="fa fa-hand-o-right fontSizeChange" aria-hidden="true"></i> OBIEE Administration</li>
            <li><i class="fa fa-hand-o-right fontSizeChange" aria-hidden="true"></i> OBIEE Consulting</li>
        </ul>
        <br>
        <!--/.our-skill--><!-- our-team -->
        <!--section-->
    </div>
    <!--========== END PAGE CONTENT ==========-->
    <?php
        include "footer.php";
    ?>
</body>
<!-- End Body -->
</html>
